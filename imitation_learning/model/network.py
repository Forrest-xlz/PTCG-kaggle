...
class PTCGTransformer(torch.nn.Module):
    def __init__(self, config: ModelConfig):
        ...
        layer = torch.nn.TransformerEncoderLayer(config.d_model, config.num_heads, config.d_feedforward, dropout=0)
        self.encoder = torch.nn.TransformerEncoder(layer, config.encoder_layers, enable_nested_tensor=False)
        self.decoder_bag = torch.nn.EmbeddingBag(config.decoder_size, config.d_model, mode="sum")
        self.decoder = torch.nn.ModuleList(
            DecoderLayer(config.d_model, config.num_heads, config.d_feedforward)
    ...
    def forward(self, index_encoder, value_encoder, offset_encoder, index_decoder, value_decoder, offset_decoder):
        ...
        policy = self.decoder_bag(index_decoder, offset_decoder, value_decoder)
