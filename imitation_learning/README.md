...
`recent_dates` selects the newest cached dates, including the latest date. For each date
...
the replay/sample counts. After the one-time two-player extraction and
...
ordered top decks live in `cfg/validation.yaml`. The standalone command prints CE loss
...
`log_every_steps` and `save_every_steps` both use successful
